...
int triggerSignals_C2() {
    ...
    // Substituir este comentário pelo código da função a ser implementado pelo aluno
    signal(SIGUSR2, trataSinalSIGUSR2_C10);
    signal(SIGINT, trataSinalSIGINT_C11);
...
MsgContent getDadosPedidoUtilizador_C3() {
    ...
    if (&msg.msgData.infoLogin.nif > 0 && strlen(msg.msgData.infoLogin.senha) > 0) {
...
int sendClientLogin_C4(MsgContent msg) {
    ...
    // Substituir este comentário pelo código da função a ser implementado pelo aluno
    msgId = msgGet(); // Obter o ID da mensagem

    // Enviar a mensagem do tipo MSGTYPE_LOGIN para a MSG
    if (msgSend(msgId, msg, MSGTYPE_LOGIN) == RET_ERROR) {
        so_error("C4", "");
    } else {
        so_success("C4", "");
        result = RET_SUCCESS;
...
int receiveProductList_C6() {
    ...
    if (msgReceive(msgId, &msg, getpid()) == -1) {
        so_error("C6", "");
        return result;
    } else {
    ...
    // C6.2 Valida se o resultado da autenticação do Servidor Dedicado foi sucesso. Se não foi sucesso, dá so_error e termina o Cliente. Senão, escreve no STDOUT a frase “Lista de Produtos Disponíveis:”.
    ...
    while (1) {
        ...
        if (msg.msgData.infoProduto.idProduto == FIM_LISTA_PRODUTOS) {
            // Se o campo idProduto tiver o valor FIM_LISTA_PRODUTOS, convencionou-se que significa que não há mais produtos a listar.
            so_success("C6.3", "");
            return RET_SUCCESS;  // Retorna sucesso (vai para C7).
        }

        // C6.4 Mostra no STDOUT uma linha de texto com a indicação de idProduto, Nome, Categoria e Preço.
        printf("ID: %d, Nome: %s, Categoria: %s, Preço: %d\n", msg.msgData.infoProduto.idProduto, msg.msgData.infoProduto.nomeProduto, msg.msgData.infoProduto.categoria, msg.msgData.infoProduto.preco);
...
int sendClientOrder_C8(int idProduto, int pidServidorDedicado) {
    ...
    // Enviar a mensagem para o servidor dedicado
    if (msgSend(msgId,msg, pidServidorDedicado) != -1) {
        so_success("C8","");
        result = RET_SUCCESS;
    }
...
void receivePurchaseAck_C9() {
    ...
    // Substituir este comentário pelo código da função a ser implementado pelo aluno
    // Ler a mensagem do tipo PID do processo Cliente com a resposta final do Servidor Dedicado
    int bytesLidos = msgReceive(msgId, &msg, getpid());
    if (bytesLidos == -1) {
        so_error("C9","");
        exit(RET_ERROR);
    }
    ...
    if (msg.msgData.infoProduto.idProduto == -1) {
        so_error("C9","");
        printf("Ocorreu um problema na sua compra. Tente novamente\n");
    } else if (msg.msgData.infoProduto.idProduto == PRODUTO_COMPRADO) {
